...
5. Provides a headless CLI pipeline (`main.py`) fully compatible with OSCAR/SLURM, a
   local court-corner setup helper (`tools/prepare_run.py`), and a Streamlit results dashboard
   (`tools/dashboard.py`) that reads output JSON artifacts.
...
`tools/prepare_run.py` owns all GUI interaction and always saves to
...
`tools/dashboard.py` reads `events.json`, `rally_data.json`, and `analytics.json` from a
