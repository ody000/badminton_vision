...
Uses the working pretrained weights from slayminton/models/tracknet.pt.
...
###############################################################################
# ARCHIVED: Previous "corrected" architecture (NOT compatible with weights)
# Kept for reference. Do NOT use this unless you have weights trained with it.
###############################################################################
"""
class Conv_OLD(nn.Module):
...
class TrackNet_OLD(nn.Module):
    def __init__(self, out_channels=3):
        super(TrackNet_OLD, self).__init__()

        # Encoder: 9-channel input (3 stacked RGB frames)
        self.conv2d_1 = Conv_OLD(9, 64)
        self.conv2d_2 = Conv_OLD(64, 64)
        self.maxpool1 = nn.MaxPool2d(kernel_size=2, stride=2)

        self.conv2d_3 = Conv_OLD(64, 128)
        self.conv2d_4 = Conv_OLD(128, 128)
        self.maxpool2 = nn.MaxPool2d(kernel_size=2, stride=2)

        self.conv2d_5 = Conv_OLD(128, 256)
        self.conv2d_6 = Conv_OLD(256, 256)
        self.conv2d_7 = Conv_OLD(256, 256)
        self.maxpool3 = nn.MaxPool2d(kernel_size=2, stride=2)

        self.conv2d_8 = Conv_OLD(256, 512)
        self.conv2d_9 = Conv_OLD(512, 512)
        self.conv2d_10 = Conv_OLD(512, 512)

        # Decoder with skip connections
        self.upsample1 = nn.Upsample(scale_factor=2, mode='nearest')
        self.conv2d_11 = Conv_OLD(768, 256)   # 512 + 256 (skip)
        self.conv2d_12 = Conv_OLD(256, 256)
        self.conv2d_13 = Conv_OLD(256, 256)

        self.upsample2 = nn.Upsample(scale_factor=2, mode='nearest')
        self.conv2d_14 = Conv_OLD(384, 128)   # 256 + 128 (skip)
        self.conv2d_15 = Conv_OLD(128, 128)

        self.upsample3 = nn.Upsample(scale_factor=2, mode='nearest')
        self.conv2d_16 = Conv_OLD(192, 64)    # 128 + 64 (skip)
        self.conv2d_17 = Conv_OLD(64, 64)
